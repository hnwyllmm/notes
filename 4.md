...
考虑过一个启发式方法来减少join order的排列分布。When possible, the search is reduced by consideration

only of join orders which have

join predicates relating the inner relation

to the other relations already participating

in the join.This means that in joining

relations tl,t2,...,tn only those orderings

til,ti2,...,tin are examined in which for
