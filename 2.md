...
树状结构查询评估计划通过deman-driven dataflow来执行查询。（The tree-structured query evaluation plan is used to execute queries by  demand-driven  dataflow.）next操作的返回值，除了一个status标识，还有一个叫做Next-Record的结构，包含一个RID和一个buffer pool中的record地址。这个record在buffer中pin住了。fixing和unfixding record的协议是这样的。在任何一个时间点，每个在buffer中的record仅被一个operator拥有。接收一个record后，operator可以拿一会儿，比如在hash 表中，然后放开它(unfix it)，比如一个谓词返回错误，或者将它传递到下一个operator。创建新record的复杂操作，比如join操作，必须在传送它们之前将输出的record固定在buffer中，然后放开（unfix）它们的输入record。因为这会产生大量的buffer 调用（查询中每个operator每次一个record），buffer manager的接口重新设计了，要求在生产侧每个cluster一共两个buffer调用（比如file scan)，不管一个cluster包含多少个record，并且在消费侧每个cluster只有一次buffer调用。
